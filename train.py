...
checkpoint = ModelCheckpoint(os.path.join('/tmp/shshi/checkpoint', ckpt), monitor='val_loss', save_weights_only=False, verbose=1, save_best_only=True)
...
am.ctc_model.fit_generator(batch, steps_per_epoch=batch_num, epochs=epochs, callbacks=[checkpoint], workers=8, use_multiprocessing=True, validation_data=dev_batch, validation_steps=200)
am.local_model.save_weights('logs_am/%s_model.h5' % EXPERIMENT)
