...
if True:
    ...
    parser.add_argument('--training_models', type=str, default='am,lm', help='Specify the models for training: am,lm or am or lm')
...
training_am = False
training_lm = False
training_models = args.training_models.split(',')
if 'am' in training_models:
    training_am = True
if 'lm' in training_models:
    training_lm = True
...
if training_lm:
    ...
    with tf.Session(graph=lm.graph, config=tf.ConfigProto(log_device_placement=True)) as sess:
        ...
        saver.save(sess, '%s/lm%s_model_%d' % (args.saved_dir, EXPERIMENT, epochs + add_num))
        #writer.close()
